# In restorations/deep_learning/rrdbnet_arch.py
# (This is the FIRST version, which is correct for your weights)
...
class RRDBNet(nn.Module):
    def __init__(self, num_in_ch, num_out_ch, num_feat, num_block, num_grow_ch, scale=4):
        super(RRDBNet, self).__init__()
        self.scale = scale
        self.conv_first = nn.Conv2d(num_in_ch, num_feat, 3, 1, 1, bias=True)
        self.RRDB_trunk = make_layer(lambda: RRDB(num_feat, num_grow_ch), num_block)
        ...
        if self.scale == 4:
            ...
            self.upconv4 = nn.Conv2d(num_feat, num_feat, 3, 1, 1, bias=True)
    ...
    def forward(self, x):
        ...
        if self.scale == 2:
            ...
            fea = self.lrelu(self.upconv4(fea))
